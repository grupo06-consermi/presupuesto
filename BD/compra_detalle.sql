...
BEGIN
	SELECT comp_cod, prod_cod, cdet_cantidad, cdet_total, cdet_importe
	FROM compra_detalle comp
		INNER JOIN compra comp ON comp.comp_cod = comp.comp_cod
		INNER JOIN producto prod ON comp.prod_cod = prod.prod_cod
	WHERE comp.comp_cod = _comp_cod
		AND comp.prod_cod = _prod_cod;
END $$
